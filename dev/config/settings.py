...
CONE_BLUE_HSV_LOW = (70, 30, 150)           # 蓝色锥桶 HSV 下限【现场可调】（挡板是 (95,80,60)）
                                            # ★ V≥150 是 2026-09-23 实验室实测加的：锥桶是**亮**青蓝
                                            #   （V 中位 173~180）；画面底边的暗青蓝物体/阴影（V≈100）
                                            #   会误检成锥桶（实验室无锥桶 31 帧误检 22 帧 → 0 帧）
...
# 连续被拒多少次就认定"误差真的换了水平"（清窗口接受新值）。2026-09-22 落地实测加的：
# 原版"离群就永久沿用旧值"会让滤波器在误差阶跃后**永久冻结**（实测冻了 1.6s，
# 期间车偏了十几厘米而控制器毫无反应、跑偏保护也不触发）。4 帧 @15FPS ≈ 0.27s。
ERROR_FILTER_MAX_REJECTS = 4
