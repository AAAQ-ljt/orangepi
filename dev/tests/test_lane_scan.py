...
def test_single_side_line_gives_zero_confidence():
    ...
    # HSV 通道的 valid_rows 计"至少一侧有线的行"（单侧帧可能 >0）；
    # 关键保证是**双侧齐全行为 0 → 置信度 0**，中心绝不由单侧行给出。
    assert obs.center_x == _scanner().target_x, "单侧线时不得输出可信中心"


def test_wide_white_region_is_not_a_lane_line():
    """大片白色区域（实验室地面/纸边）不应被当成车道线。"""
    frame = _track_image()
    # 填满整个扫线 ROI（0.35H ~ H-margin）：整块白是"面"不是"线"，
    # 形状过滤的厚度阈值（LANE_LINE_MAX_THICK_PX）必须把它挡住。
    frame[int(H * settings.LANE_ROI_TOP_RATIO):H - 16, :] = 235
